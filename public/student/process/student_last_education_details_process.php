...
if(is_post_request()){
  ...
  $args = $_POST;
    ...
    $subadmin = StudentEducationDetails::find_by_attribute($set = ['attribute' => 'fk_id', 'id' => $fk_id]);

    if($subadmin == false){
      // echo 'NOT MERGED'.PHP_EOL;
  $subadmin = new StudentEducationDetails($args);
  }else{
      // echo 'MERGED'.PHP_EOL;
  ...
  if($result === true) {
    $new_id = $subadmin->id;
    // write to a log file
    $logger->get_activity_logger('Education Details','Education Details was successfully filled by ID ='. $new_id . ' on '.date("j F Y, g:i:s A") . ' by ' . $session->get_id());
